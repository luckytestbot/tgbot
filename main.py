...
async def handle_token(data):
    ...
    # Nowy warunek: czy initial_buy_percentage jest bliski liczbie całkowitej
    is_close_to_integer = abs(initial_buy_percentage - round(initial_buy_percentage)) <= 0.02

    if not is_close_to_integer:
        print(f"Initial buy {initial_buy_percentage:.2f}% nie jest zbliżony do liczby całkowitej. Pomijam.")
        return
