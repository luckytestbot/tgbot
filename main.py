...
last_seen_cas = set()
last_seen_cas_lock = asyncio.Lock()  # Blokada do synchronizacji dostępu do last_seen_cas

# Funkcja formatująca czas na "dd-mm-yyyy HH:MM"
def format_simple_datetime(dt):
    return dt.strftime("%d-%m-%Y %H:%M")

# Funkcja pobierająca liczbę tokenów dla danego twórcy
...
# Funkcja do pobierania czasu najstarszej transakcji
async def get_oldest_transaction_time(dev_address):
    attempts = 0
    while attempts < 3:
        try:
            limit = 100 if attempts == 1 else 50
            payload = {
                "jsonrpc": "2.0",
                "id": "helius",
            ...
            if not transactions:
                ...
                if attempts == 0:
                    print("Próba z większym limitem...")
...
# Funkcja przypisująca emoji do tokena na podstawie różnicy czasu
...
# Funkcja przetwarzająca nowy token
...
# Funkcja do nasłuchiwania tokenów z WebSocket
