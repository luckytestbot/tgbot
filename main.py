...
def get_oldest_transaction_time(dev_address):
    ...
    while attempts < 3:
        try:
            # Pierwsza próba z limitem 50 transakcji
            payload = {
                "jsonrpc": "2.0",
                "id": "helius",
            ...
            if not transactions:
                ...
                # Jeśli brak transakcji, po 0.5 sekundy próba z limitem 100
                if attempts == 0:
                    print("Próba z większym limitem...")
                    time.sleep(0.5)  # Opóźnienie 0.5 sekundy przed kolejną próbą
                    attempts += 1
                    continue  # Przechodzimy do kolejnej próby
                return None
